...
const UpdateToken = ({ handlevisibility }) => {
  const {id} = useParams();
  ...
  const handleAddTokenClick = async (e) => {
    e.preventDefault();
    const tokenValues = {
    ...
    try {
      const response = await axios.put(`http://localhost:8081/getTokenData/edit/${id}`, formData)
      if(response) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8081/getTokenData/edit/${id}`)
      .then(res => {
        console.log(res.data)
        const data = res.data[0];
        // Set the form data with the retrieved data
        setFormData(data);
      })
          ...
          <Row>
            <Col>
              ...
              <Form.Group controlId="reportedDate">
                ...
                <Form.Control
                  type="date"
                  value={formData.reportedDate.split('T')[0]}
                  onChange={handleInputChange}
                />
              </Form.Group>
            ...
            <Col>
              ...
              <Form.Group controlId="updatedDate">
                ...
                <Form.Control
                  type="date"
                  value={formData.updatedDate.split('T')[0]}
                  onChange={handleInputChange}
                />
              </Form.Group>
